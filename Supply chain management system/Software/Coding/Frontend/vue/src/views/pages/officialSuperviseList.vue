...
<!--编辑弹窗-->
    ...
    >
            <el-form status-icon label-width="100px" ref="formRef" :model="allData.editData" >
              <el-form-item label="检测项目" prop="saleTestProject">
                <el-input v-model="allData.editData.saleTestProject" placeholder="请输入检测项目"  />
              </el-form-item>
              <el-form-item label="检测结果" prop="saleTestResult">
                <el-input v-model="allData.editData.saleTestResult" placeholder="请输入检测结果"  />
              </el-form-item>
              <el-form-item label="收货时间" prop="saleTime">
                <el-date-picker
              ...
              <el-form-item label="检测时间" prop="saleTestTime">
                <el-date-picker
                    v-model="allData.editData.saleTestTime"
                    type="date"
                    placeholder="Pick a date"
                    format="YYYY/MM/DD"
                    value-format="YYYY-MM-DD"
...
function openEdit(index,row){
  ...
  router.push({
    path:'/officialSuperviseEdit',
    query: {key: row.breedOnlyKey,name:row.productName}
  })
}
//编辑处理
function editData(){
  ...
  baseData
      .editSale({...allData.editData,saleOnlyKey:allData.editData.breedOnlyKey})
      .then((res) => {
        this.getData();
        if(res.data.result){
