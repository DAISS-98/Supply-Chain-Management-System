...
<!--  数据展示  -->
    ...
    <h2 style="color: #999999">官方抽检信息</h2>
    <el-divider />
    <el-form status-icon label-width="100px" ref="formRef" :model="allData.form" >
      <el-form-item label="抽检对象" prop="officialProduct">
        <el-input v-model="allData.form.officialProduct" placeholder="请输入抽检对象" maxlength="20" />
      </el-form-item>
      <el-form-item label="报告日期" prop="officialDate">
        <!--        <el-input v-model="allData.form.breedTime" placeholder="请输入种养殖时间"  />-->
        <el-date-picker
            v-model="allData.form.officialDate"
            type="date"
      ...
      <el-form-item label="报告结论" prop="officialConclusion">
        <el-input type="textArea" v-model="allData.form.officialConclusion" placeholder="请输入报告结论" maxlength="500"  />
      </el-form-item>

      <el-form-item v-if="allData.form.officialFileGuid != undefined" label="报告上传" prop="officialFileGuid">
        <upload-file @getGuid="getSelfTestGuid"  :fileGuid="allData.form.officialFileGuid">
        </upload-file>
      </el-form-item>
    </el-form>
...
function getSelfTestGuid(res){
  if(res == null || res == ''){
    allData.form.officialFileGuid="1"
  }else {
    allData.form.officialFileGuid=res
  }
}
...
function getData() {
  ...
  baseData
      .officialSuperviseSelectOne({officialOnlyKey:key.value})
      .then((res) => {
        allData.form = (res.data.data[0]!=null)?res.data.data[0]:{}
        console.log(allData.form)
        allData.form.productName = productName;
        allData.form.officialFileGuid = (allData.form.officialFileGuid != null)?allData.form.officialFileGuid:1
        allData.form.officialDate = (allData.form.officialDate != null)?timestampToTime(allData.form.officialDate):null
      })
      .catch(() => {
...
function back(){
  router.push('/officialSuperviseList');
  // router.push("/")
  // router.go(-1);
}
...
function editData(){
  ...
  baseData
      .officialSuperviseInsertOrEdit({...allData.form,officialOnlyKey:key,userId:userId})
      .then((res) => {
        if(res.data.result){
